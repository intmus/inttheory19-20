...
As we determined in [Unit 2c]({{ site.baseurl }}/02-int-scales-keys/c1-keys.html), you only need to alter half-step in the circle-of-fifths to create a diatonic collection out of the chromatic collection <!-- need connecting sentence here --> When grouped together, the diatonic scales are simply a collection of five whole steps and two half-steps. But when you study the examples below, you will see that the half-steps are the foundation of voice-leading in diatonic progressions. If you understand the voice-leading principles that pull the V chord into the I chord, you can then extend these rules to create the basic progression from which all diatonic harmony evolves.
